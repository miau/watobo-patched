    ...
    class ProxyDialog < FXDialogBox
     #TODO: Implement Connection Check!!
      ...
      def switchEnableProxy()
        if not @proxyEnableCheckButton.checked? then
          @proxyTable.hide()
          @newProxyButton.disable
          @delProxyButton.disable
          @editProxyButton.disable
          @acceptBtn.enable
          @proxy = nil
        @proxyTable.killSelection()
        else
          @proxyTable.show()
        ...
        @proxySelectionFrame.enabled = @proxyEnableCheckButton.checked?
        @proxy_prelabel.enabled = @proxyEnableCheckButton.checked?
        @proxy_label.enabled = @proxyEnableCheckButton.checked?
      end

      def refreshProxyList
        @proxyTable.clearItems(false)
        @proxyTable.setTableSize(0, 2)
